...
export const Post = () => {
    ...
    return (
        ...
        <div className="md:flex">
            <div className="p-8">
                <div className="uppercase tracking-wide text-sm text-indigo-500 font-semibold">{postDetail.Title}</div>
                <p className="mt-2 text-gray-500">{postDetail.Body}</p>
                <div className="mt-4">
                    <span className="text-gray-600 text-sm">Posted by User {postDetail.UserId}</span>
                </div>
                <div className="mt-4 text-gray-500 text-xs">
                    ...
                    {postDetail.DeletedAt ? (
                        <p className="text-red-500">Deleted At: {new Date(postDetail.DeletedAt).toLocaleString()}</p>
                    ) : null}
                </div>
            </div>
        </div>
